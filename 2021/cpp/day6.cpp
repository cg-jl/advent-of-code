...
auto read_input(std::string_view line) -> std::vector<std::size_t> {
    ...
    for (std::size_t found = line.find(','); found != line.npos; found = line.find(',')) {
...
auto day(std::vector<std::size_t>& world) -> void {
    const auto current_size = world.size();
...
auto  main() -> int {
    ...
    std::cout << std::accumulate<decltype(world)::const_iterator, std::size_t>(world.cbegin(), world.cend(), 0) << '\n';
    return 0;
}